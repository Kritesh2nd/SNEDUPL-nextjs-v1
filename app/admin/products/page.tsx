...
export default function AdminProductsPage() {
  ...
  const openEdit = (idx: number) => {
    ...
    setForm({
      ...
      origin: p.origin,
      tasteNotesRaw: p.tasteNotes.map((n) => n.label).join(", "),
      image: p.image,
      featured: p.featured,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">
            {siteContent.products.length} total products
          </p>
        </div>
      ...
      >
        <table className="w-full text-sm">
          <thead>
            ...
            >
              {["Image", "Name", "Category", "ABV", "Featured", "Actions"].map(
                (h) => (
                  <th
                    key={h}
                    className="px-4 py-3 text-left text-[10px] text-white/40 uppercase tracking-widest font-semibold"
                  >
                    {h}
                  </th>
                ),
          ...
          <tbody>
            {products.map((product) => {
              const ri = siteContent.products.findIndex(
                (p) =>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className="text-[9px] px-2 py-0.5 rounded-full tag-green">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-[10px] ${product.featured ? "text-[var(--g400)]" : "text-white/25"}`}
                    >
      ...
      <Modal
        ...
        title={editIndex !== null ? "Edit Product" : "Add Product"}
        maxWidth="max-w-2xl"
      >
        <div className="space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) =>
                set("alcoholPercent", parseFloat(e.target.value) || null)
              }
              placeholder="e.g. 25"
            />
            <Input
          ...
          <Input
            label="Taste Notes (comma-separated)"
            value={form.tasteNotesRaw}
            onChange={(e) => set("tasteNotesRaw", e.target.value)}
            placeholder="Smooth, Crisp, Clean"
            helpText="Separate each note with a comma"
          />
          <div className="flex gap-6">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) => set("isUpcoming", e.target.checked)}
                className="accent-[var(--g600)]"
              />
        ...
        <div className="text-center space-y-4">
          <p className="text-white/55 text-sm">
            ...
            <span className="text-white">
              {deleteConfirm !== null
                ? siteContent.products[deleteConfirm]?.name
                : ""}
            </span>
