...
export default function ProductsSection() {
  ...
  const filtered = filter === "ALL" ? siteContent.products : siteContent.products.filter((p) => p.category === filter);

  return (
    <section id="products" className="relative py-24 md:py-32" style={{ background:"var(--dark)" }}>
      <div className="absolute left-0 top-1/4 w-80 h-80 pointer-events-none" style={{
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <Link href="/products" className="flex-shrink-0">
            <Button variant="outline" size="sm">View All Products <ArrowRight size={13} /></Button>
          </Link>
        </div>
          ...
          {FILTERS.map((f) => (
            <button key={f.value} onClick={() => setFilter(f.value)}
              ...
              style={filter === f.value
                ? { background:"var(--g400)", boxShadow:"0 0 15px rgba(74,222,128,0.3)" }
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {filtered.map((product, i) => (
            <div key={`${product.name}-${i}`} className="fade-up" style={{ animationDelay:`${i * 0.05}s`, animationFillMode:"both" }}>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20 text-white/25">
            <p className="text-sm tracking-wider uppercase">No products in this category</p>
          </div>
        )}
      </div>
      ...
      {selected && (
        <Modal open={!!selected} onClose={() => setSelected(null)} title={selected.name} maxWidth="max-w-lg">
          <div className="space-y-4">
            <span className="text-[10px] tracking-widest uppercase" style={{ color:"var(--g400)" }}>
