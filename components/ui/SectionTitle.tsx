...
export default function SectionTitle({ eyebrow, title, subtitle, centered, accentColor = "green" }: Props) {
  const eyebrowColor = accentColor === "green" ? "var(--g400)" : "var(--b400)";
  const dividerGradient = accentColor === "green"
  ...
  return (
    <div className={`flex flex-col gap-3 ${centered ? "items-center text-center" : ""}`}>
      {eyebrow && (
        <span className="text-[11px] font-semibold tracking-[0.25em] uppercase" style={{ color: eyebrowColor }}>
          {eyebrow}
        </span>
      )}
      ...
      {subtitle && (
        <p className="text-sm md:text-base leading-relaxed max-w-2xl text-white/50">{subtitle}</p>
      )}
      <div
